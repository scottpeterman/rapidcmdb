...
@topology_bp.route('/api/topology/export/drawio', methods=['POST'])
def api_export_drawio_new():
    """Export topology to Draw.io format using the new NetworkDrawioExporter"""
    try:
        config = request.get_json() or {}

        # Use the same filters as the current topology view
        sites = request.args.getlist('site') or config.get('sites', [])
        roles = request.args.getlist('role') or config.get('roles', [])
        ...
        # Get layout and other export options
        raw_layout = config.get('layout', 'tree')
        include_endpoints = not network_only  # If network_only=True, exclude endpoints
        use_icons = config.get('use_icons', True)
        icons_dir = config.get('icons_dir', './icons_lib')

        # Convert layout parameter
        if raw_layout in ['TD', 'TB', 'BT', 'hierarchical']:
            layout = 'tree'
        elif raw_layout in ['LR', 'RL']:
            layout = 'tree'
        elif raw_layout == 'balloon':
            layout = 'balloon'
        else:
            layout = 'tree'

        logger.info(
            f"Draw.io export with new exporter: sites={sites}, network_only={network_only}, "
            f"layout={layout} (from {raw_layout}), include_endpoints={include_endpoints}")
        ...
        # Create output path
        output_filename = f"network-topology-{timestamp}.drawio"
        output_path = Path(output_filename)

        # Create and configure the new exporter
        ...
        logger.error(f"Draw.io export error with new exporter: {e}", exc_info=True)
        return jsonify({
            'success': False,
            'error': str(e),
            'message': 'Export failed using new Draw.io exporter'
        }), 500
